...
const ensureScanToken = (request: NextRequest): void => {
  const requiredToken = process.env.QC_DISPATCH_ALERT_SCAN_TOKEN?.trim();
  if (!requiredToken) {
    return;
  }

  const incomingToken = request.headers.get("x-qc-alert-scan-token")?.trim();
...
// POST /api/qc/dispatch-alerts/scan - 执行跨书籍告警扫描任务
export const POST = withErrorHandler(async (request: NextRequest) => {
  ensureScanToken(request);

  const body = await request.json().catch(() => ({}));
  const { searchParams } = new URL(request.url);
  const query = parseQcDispatchAlertScheduleQuery(searchParams);
  const bookIds = parseBookIdsFromRequest(searchParams, body);
  const triggeredBy = request.headers.get("x-operator") || "scheduled_scan_api";
