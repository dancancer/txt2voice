...
import { checkScriptCoverage, validateStructuredOutput } from "../tools/validation-tools";
import {
  ...
  type ScriptProductionBookSegment,
...
interface ScriptProductionBook {
  id: string;
...
interface SegmentSuccessResult {
  status: "success";
...
interface SegmentFailureResult {
  status: "failed";
  failure: SegmentFailureDetail;
  counters: SegmentRuntimeCounters;
}

type SegmentRunResult = SegmentSuccessResult | SegmentFailureResult;

type ScriptProductionWorkflowResult = GeneratedScript & {
...
const createEmptySegmentCounters = (): SegmentRuntimeCounters => ({
...
const isSegmentScriptDraft = (
  value: SegmentFailureDetail | SegmentScriptDraft
): value is SegmentScriptDraft =>
  Array.isArray((value as SegmentScriptDraft).lines) &&
  typeof (value as SegmentScriptDraft).createdAt === "string";

const runFormatRepair = async (params: {
  ...
  runSegmentRepairStage: typeof runSegmentRepairStage;
  failedStage: Exclude<RunSegmentScriptingStageResult, { status: "completed" }>;
}): Promise<SegmentScriptDraft | SegmentFailureDetail> => {
  const repairStage = await params.runSegmentRepairStage({
    workflowRunId: params.workflowRunId,
    segmentId: params.segment.id,
    segmentText: params.segment.content,
    failureKind: "format_repair",
    failedArtifact: resolveFailureArtifact(params.failedStage),
    repairDepth: 0,
  ...
  return repairStage.artifact.segmentScriptDraft;
};

const runSingleSegment = async (params: {
  workflowRunId: string;
  bookId: string;
  segment: ScriptProductionBookSegment;
  adapter: LLMAdapter;
  runtimeStore: ScriptProductionRuntimeStore;
  characterProfiles: ScriptProductionBook["characterProfiles"];
  ...
  createToolCall: (
    record: import("./run-agent").ToolCallRecord & { createdAt?: Date }
  ) => Promise<void>;
  updateToolCall: (
    record: import("./run-agent").ToolCallRecord & { completedAt?: Date }
  ) => Promise<void>;
  appendTrace: (event: ExecutionEvent) => Promise<void>;
  runSegmentScriptingStage: typeof runSegmentScriptingStage;
  runSegmentRepairStage: typeof runSegmentRepairStage;
  ...
  const scriptStage = await params.runSegmentScriptingStage({
    workflowRunId: params.workflowRunId,
    segmentId: params.segment.id,
    segmentText: params.segment.content,
    adapter: params.adapter,
  ...
  if (!validationReport.valid) {
    ...
    if (canAttemptInputRefinement) {
      const refinementStage = await params.runSegmentRepairStage({
        workflowRunId: params.workflowRunId,
        segmentId: params.segment.id,
        segmentText: params.segment.content,
      ...
      if (
        ...
        if (refinementStage.agentRunId) {
          ...
          await params.runtimeStore.updateToolCall({
            ...
            toolName: "split-segment",
            status:
              refinedSegments.length > 1 ? "completed" : "failed",
            resultSummary: {
  ...
  const qualityStage = await params.runQualityStage({
    workflowRunId: params.workflowRunId,
    segmentId: params.segment.id,
  ...
  const persistStage = await params.runPersistStage({
    workflowRunId: params.workflowRunId,
