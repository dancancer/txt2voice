...
1. 冻结 Annotation v2 字段清单与 JSON Schema。  
2. 输出 Prisma 迁移草案（新增 6 张质量与复核相关表）。  
3. 完成 `AUTO_PIPELINE` 任务类型与 API 契约。  
4. 先上线 Q1/Q2/Q3 的 Fast Gate（可配置阈值）。  
5. 建立人工复核最小页面（列表 + 试听 + 重生）。
